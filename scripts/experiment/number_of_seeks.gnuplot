...
plot [0-4*a:5+5*a][1:]\
                 './data/creads/creads_ssd.dat'\
		                      using (0-2*a):4 w boxes lt 3 title "Naive blocks - real",\
    	         './data/seeks-model.dat'\
				      using (0-a):1 w boxes lt 3 fs transparent title "Naive blocks - model",\
                 './data/buff-slices/buff-slices_ssd.dat'\
		                      using (0):4 w boxes lt 4 title "Naive slices - real",\
    	         './data/seeks-model.dat'\
   				      using (a):11 w boxes lt 4 fs transparent title "Naive slices - model",\
                 './data/creads/creads_ssd.dat'\
                                      using (1-2*a):9 w boxes lt 1 title "Cluster reads - real",\
                 ...
                 './data/seeks-model.dat'\
		                      using (1-a):2 w boxes lt 1 fs transparent title "Cluster reads - model",\
	   	 ...
	   	 './data/mreads/mreads_hdd.dat'\
		                      using (1):4 w boxes lt 2 title "Multiple reads - real",\
     	    			   '' using (2):9 w boxes lt 2 notitle,\
				   '' using (3):14 w boxes lt 2 notitle,\
				   '' using (4):19 w boxes lt 2 notitle,\
    	         './data/seeks-model.dat'\
				   using (1+a):7 w boxes lt 2 fs transparent title "Multiple reads - model",\
                 ...
                 './data/buff-slices/buff-slices_ssd.dat'\
			              using (1+2*a):9 lt 5 w boxes title "Buffered slices - real",\
    	                           '' using (2+2*a):14 lt 5 w boxes notitle	,\
       	                           '' using (3+2*a):19 lt 5 w boxes notitle	,\
       	                           '' using (4+2*a):24 lt 5 w boxes notitle	,\
      	         './data/seeks-model.dat'\
				   using (1+3*a):13 w boxes lt 5 fs transparent title "Buffered slices - model",\
