...
consists of a stack of one or more 2D tiles (incomplete slices or incomplete columns) spanning sequentially occuring slices, whereas a 3D slab is 
a series of one of more contiguous 2D slices. A dataset such
...
memory load only partially covers columns in the reconstructed image:
O($d^2$) seeks are then required during writing, one for every partial
column in every partial slabs. In the intermediary case, columns are
...
overlaps multiple block slabs in Case 2 or multiple block columns in
