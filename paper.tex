...
buffer are given later in the document.
\begin{figure}
\centering
...
required. See also Figure~\ref{fig:cluster-reads}. In summary:
\begin{equation}
...
\caption{Cluster reads configurations leading to different number of
  seeks.}
...
If $\frac{Rb}{\sqrt[3]{n}}>m\geq\frac{Rb}{\sqrt[3]{n}^2}$, i.e., a
full row of blocks fits in memory but a full slice does not, then a
variant of cluster reads consists in adjusting the memory load to the
highest value that makes sure that only full rows are written. In this
case:
\begin{equation}
  N_\mathrm{cluster\_reads\_v1}= n + \frac{bR}{m|}d
\end{equation}
where $m|=\frac{Rb}{\sqrt[3]{n}^2}\floor*{\frac{m\sqrt[3]{n}^2}{Rb}}$

And if $m\geq\frac{Rb}{\sqrt[3]{n}}$, i.e., a full slice of blocks
fits in memory, then a second variant adjusts
the memory load to the highest value that makes sure that only full
slices are written. In this case:
\begin{equation}
  N_\mathrm{cluster\_reads\_v2}= n + \frac{bR}{m||}d
\end{equation}
