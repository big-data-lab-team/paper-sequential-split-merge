...
\caption{Memory-load configurations in multiple reads, leading to
  different number of seeks. Red regions shows different memory
  load in each case. Small dots are used to represent block frontiers,
  long dashes used to represent rows/slices in blocks.}
...
$k<d$) or of block slices (case 5, $k<\sqrt[3]{n}$) \todo{make a
  figure}. In each of these 5 cases in Figure~\ref{fig:multiple-reads-cases}, we define $v$ as follows:
\begin{equation*}
...
$b_i'$ is the number of blocks access by the last memory load. We have:
\begin{equation*}
  x_i = \ceil*{\frac{Rb}{m_i'}} \quad ; \quad x_i -1 = \floor*{\frac{Rb}{m_i'}}, \quad i \in \llbracket 1,5 \rrbracket
\end{equation*}
and:
\begin{equation*}
...
\begin{cases}
  ...
  + \sqrt[3]{n}D^2 \text{ mod } k_1& \text{if } d \leq \frac{m}{b} < D\\
  \ceil*{\frac{Rb}{m_2'}}(\sqrt[3]{n}+1)   & \text{if } D \leq \frac{m}{b} < Dd\\
  \floor*{\frac{Rb}{m_3'}}(k_3\sqrt[3]{n}+1)+1+\\
  \quad \quad \sqrt[3]{n}\left(\sqrt[3]{n}D\text{ mod } k_3\right)&\
...
\begin{cases}
  ...
  \quad \quad \quad \sqrt[3]{n}\sqrt[3]{R}^2 \text{ mod }\floor*{\frac{m\sqrt[3]{n}}{\sqrt[3]{R}b}}\
