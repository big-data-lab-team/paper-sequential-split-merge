...
\begin{algorithm}[h]
  ...
  \begin{algorithmic}[1]
  \STATE sorted\_split\_list = sort splits based on its postion in increasing (z,y,x) values
  ...
  \WHILE{end\_index \textless sizeof(reconstructed\_image)}
    ...
    \FOR{split in sorted\_split\_list}
      \IF{split is in write\_range}
        \STATE read split
        \STATE extract the rows in the write\_range
        \STATE write rows to the data\_buffer
      \ENDIF
    \ENDFOR
    \STATE write data\_buffer continuously to reconstructed\_image
