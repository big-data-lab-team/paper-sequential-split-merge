...
\begin{abstract}
  ...
  address this issue. Cluster reads and Cluster writes access image
...
order. All voxels in a \emph{slice} have a the same $k$ and all voxels in
a \emph{row} have the same $j$.  NIfTI is such a format,
...
\begin{figure}
  ...
  \caption{Number of seeks for Cluster reads vs Multiple reads, for
    D=3458 and b=2. Left: n=125; Right: n=64,000. Case 1 and Case 2
    denote Cluster read configurations.}
...
the availability of so-called array proxies in Nibabel, which help to
read specific sub-parts of large images. Nibabel's array proxies
essentially provided the buffer implementation for splitting
algorithms. Unfortunately, they are not available to write
data. \todo{Yongping, Valerie: any other comment on split
  implementations?}

% Additional optimizations.
In Multiple reads, for faster reading all the blocks, before the
merging process start, we read all the block headers to get their
position information and calculate each corresponding row index in the
reconstructed image. We only read the block when there is a row in
that writing range (available memory), otherwise, we skip it.
\todo{Valerie, Yongping, any other implementation trick to report?}
...
seeks in Cluster reads is 4 orders of magnitude higher than for
Multiple reads, and 5 orders of magnitude higher than naive
slices. This huge difference comes from the fact that at 3~GB Cluster
reads are in case 1. For 6~GB, 9~GB and 12~GB, Cluster reads are in
...
\todo{ The measured seek times correlated with
the measured merging times ($\rho$=x), which suggest that the
observed time reductions were indeed caused by reduced seek times.}

% Slices and blocks
Figure~\ref{fig:merge-time} shows the merge time for all algorithms by
memory values. Naive blocks take about 5 times longer than naive
slices to merge the data, which quantifies the effect of the problem
targeted by our algorithms. This effect appears both on HDD and SSD,
which shows that SSDs might not be the only solution to reduce seek
time. In the remainder we use naive blocks and naive slices and
references to evaluate our algorithms.
...
% Cluster reads
Cluster reads provide important speed ups compared to naive blocks,
both on HDD and on SSD. On SSD, they are up to 5 times faster than
naive blocks at 16~GB, and 3 on average \todo{compute the actual
  values}. \todo{On HDD, \ldots} Surprisingly, they perform
substantially faster than naive blocks even at 3~GB, while in case
1. This may be explained by the fact that the seeks required to write
incomplete block rows to the reconstructed image are shorter than the
ones for naive blocks.

% Multiple reads
Multiple reads are even faster than Cluster reads. On SSD, they are 5
times faster than naive blocks on average \todo{compute the actual
  values} ( \todo{On HDD, \ldots}). At 12~GB, the total merging time
is only 1.2 times slower than naive slices, which solves our initial
problem to a large extent. Even at 3~GB the speed-up is considerable
compared to naive blocks.
...
\todo{define cluster writes and multiple writes somewhere.}

The number of seeks for Cluster writes and Multiple writes were
identical to the ones in Cluster reads and Multiple reads. \todo{Check
...
% library. Cluster reads could be useful in some cases. 
