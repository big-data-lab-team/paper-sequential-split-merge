...
\begin{abstract}
  ...
  parts. Cluster reads access image splits only once while Multiple
...
In our context, split and merge relate to the same dual problem . We
focus here on merging for the sake of concision. Splitting algorithms
can be derived from merging by swapping reads and writes. Our goal
then is to merge a set of $n$ chunks into a single reconstructed 3D
image with $R$ voxels of size $b$. We assume that all chunks are of
...
written after alphanumerical sorting of their (z,y,x)
representation. That is, z slices are written one after the other, and
in every z slice, y rows are written one after the other. x is the
fastest-changing dimention.  NIfTI, the file format defined by the
Neuroimaging Informatics Technology Initiative
NIfTI\footnote{\url{https://nifti.nimh.nih.gov}} is such a format,
used by the vast majority of the neuroimaging community. In the
remainder, we use the term ``slice" for ``z slice".
...
For the sake of mathematical simplicity, we assume that the
reconstructed image and blocks are cubic and that they do not
overlap. Under this assumption, we also have the following relations:
\begin{itemize}
...
read and write rates and its seek time. We do not distinguish between
access time and seek time. We also assume that seeks require a
constant amount of time, regardless of the position seeked to. In
practice, large variations would be expected, but modeling such
...
\end{equation*}
or, using $R$ and $n$ as our main variables:
\begin{equation}
...
\begin{figure}
  ...
  \begin{minipage}[b]{0.3\columnwidth}
    ...
    \caption{Buffer used in cluster reads.}
...
\begin{algorithm}[h]
  ...
  \begin{algorithmic}[1]
    \STATE sorted\_slices = sort slices by increasing z values
  ...
  \end{algorithmic}
\end{algorithm}
The number of seeks obtained with buffered slices is:
\begin{equation}
N_\mathrm{buff\_slices} =  n + \ceil*{\frac{bR}{m}}
\end{equation}
The next two algorithms attempt to generalize buffered slices to blocks.
...
image. It shows how the two first 4x4 blocks of a reconstructed image
would be arranged in the buffer.  White portions in the buffer are not
allocated.  The column-major order is used, similar to the NIfTI
format: i is the fastest changing dimension, followed by j, followed
by k. 
...
\begin{algorithm}[h]
  ...
  \begin{algorithmic}[1]
    \STATE sorted\_splits = sort splits by increasing (z,y,x) values
    \STATE initialize buffer
    \STATE (m',case)=\texttt{switch}(m,n,R,b)
    \STATE old\_block = sorted\_splits[0]
    \FOR{i = 0 ; i\textless n ; i+=1}
      \STATE block = sorted\_splits[i]
...
image and $b_i$ is the number of seeks required to write a the memory
load. The first $n$ seeks in the equation are required to read all the blocks
once. According to Figure~\ref{fig:cluster-reads}, we have:
\begin{equation*}
  b_1=d^2=\sqrt[3]{\frac{R}{n}}^2 \quad ; \quad b_2=d=\sqrt[3]{\frac{R}{n}} \quad ; \quad b_3=1
\end{equation*}

Our algorithm avoids configurations where the memory load
...
(see Figure~\ref{fig:avoided-configurations}-Right). Thus, the
...
to ensure that the memory buffer only contains contiguous bytes, at
any time. Therefore, the buffer can be written continuously to the
reconstructed image, without seeking (line 13).  It should be noted
that a given block might be read multiple times, in different memory
loads. 
...
As illustrated in Figure~\ref{fig:multiple-reads-cases}, we assume that $m'$ represents an integer number $k$
of block rows (case 1, $k<\sqrt[3]{n}$), of complete rows (case 2,
$k<d$), of tile rows (case 3, $k<\sqrt[3]{n}$), of slices (case 4,
$k<d$) or of block slices (case 5, $k<\sqrt[3]{n}$). In each of these 5 cases, we define $v$ as follows:
\begin{equation*}
...
\end{eqnarray*}
where $x_i$ is the total number of required memory loads, $b_i$ is the
...
\begin{itemize}
\item In MR, no seeking withing the splits.
...
For faster reading all the splits, before the merging process start,
