...
buffer as sequentially as possible. The buffer is a slightly complex
...
The number of seeks performed by cluster reads depends on the way that
blocks loaded in memory arrange in the reconstructed image. In the
best case, blocks in memory completely cover slices in the
reconstructed image. In this case, slices of the reconstructed images
can be written without seeking at all. If the blocks in memory
completely cover rows in the reconstructed image but do not cover
entire slices, then the blocks are written with $d$ seeks, one per
slice. And if the blocks do not cover entire rows, then they are
written with $d^2$ seeks, one per row and per slice.  In conclusion:
\begin{equation}
