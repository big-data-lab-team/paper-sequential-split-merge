...
\todo{Check that ordering in the Figure is consistent with Nifti's.}

Incomplete block rows and incomplete block slices lead to extra
seeking and should be avoided as much as possible.  Thus, cluster
reads consider only three cases, represented in
Figure~\ref{fig:cluster-reads}:
\begin{itemize}
  \item Case 1: $m<d^3\sqrt[3]{n}b=\frac{D^3}{\sqrt[3]{n}^2b}$, that is, a
    row of blocks does not fit in memory. Incomplete rows of
    blocks will be merged, with $d^2$ seeks per memory load.
  \item Case 2: $\frac{D^3}{\sqrt[3]{n}^2}b\leq m<\frac{D^3}{\sqrt[3]{n}}b$,
    ...
    $d$ seeks per memory load. We have $m'=\floor*{\frac{m}{d^3\sqrt[3]{n}b}}$.
    \item Case 3: $\frac{D^3}{\sqrt[3]{n}}b \leq m < D^3b$, that is, a slice
      of blocks fits in memory. Only complete slices of blocks
      will be merged, a single seek per memory load. We have
...
The total number of seeks performed by cluster reads depends on the three
cases listed above and on the overall number of memory loads.
\begin{equation}
