...
Multiple reads, with 3GB, 6GB, 9GB, 12GB and 16GB of memory. We also
did 0GB of memory for Buffered slices and Cluster reads, which
triggered naive slice and block merging. We did 5 repetitions for each
memory value. Memory values were shuffled in each repetition, to
correct for potential biases coming from ordering, such as caching
effects. To improve reproducibility, we dropped the kernel page,
dentry and inode caches before each run (\texttt{echo 3 | sudo tee
  /proc/sys/vm/drop\_caches}). 

\subsection{Results}

\subsubsection{Merging time}

% For each paragraph below, comment on overal trend and breakdown.
...
% Cluster reads
%% ultimately fine but not stable due to discontinuities
At 3GB, Cluster reads were in case 1, which explains that more than 10
million seeks were done, almost equivalent to naive blocks. At 6, 9
and 12~GB, Cluster reads were in case 2 and at 16~GB they were in case
3. This confirms that Cluster reads are beneficial in case 2 and 3
only, consistently with the analysis in Section~\ref{sec:analysis} .
...
\begin{figure}[h]
  ...
  \caption{Comparison between Bufered slices, naive blocks and naive slices. Left: SSD. Right: HDD.}
  % Python file: scripts/experiment/generate_avg_var_data.py
  % Gnuplot file: scripts/experiment/compare_breakdown_errbar.gnuplot
\label{fig:buffered-slices}
\end{figure}
...
\begin{figure}[h]
  ...
  \caption{Comparison between Cluster reads, naive blocks and naive slices. Left: SSD. Right: HDD.}
  % Python file: scripts/experiment/generate_avg_var_data.py
  % Gnuplot file: scripts/experiment/compare_breakdown_errbar.gnuplot
\label{fig:cluster-reads}
\end{figure}
...
\begin{figure}[h]
  ...
  \caption{Comparison between Multiple reads, naive blocks and naive slices. Left: SSD. Right: HDD.}
  % Python file: scripts/experiment/generate_avg_var_data.py
  % Gnuplot file: scripts/experiment/compare_breakdown_errbar.gnuplot
...
explains the observations.   The measured seek times correlated with
the measured merging times ($\rho$=\todo{x}), which suggest that the
...
The complete Latex sources of this paper, including the collected data
and Gnuplot scripts used to generate the results figures are available
at \url{https://github.com/big-data-lab-team/paper-sequential-split-merge} under
GPLv3 license. Our \texttt{sam} library is at
