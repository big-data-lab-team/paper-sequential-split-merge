...
naive slices is still significant (average ratio is 1.4 on SSD,
\todo{x} on HDD) although less than on HDD. Clustered writes and
Multiple writes both perform similarly to naive slices. Buffered
slices don't provide any significant speed-up compared to naive
slices.

The breakdown by read, write and overhead time is shown in
Figure~\ref{fig:breakdowns-ssd-split}. We were not able to measure
seek time for splitting algorithms as it was blended in read time by the Nibabel library. \todo{comment on the figures when results are
  available. Split time looks slightly longer than merge time.}
